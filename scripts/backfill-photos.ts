...
async function main(): Promise<void> {
  ...
  console.log(`Found ${posts.length} posts with photo_file_id`);
  ...
  for (let i = 0; i < posts.length; i++) {
    ...
    try {
      ...
      const error = err as { response?: { error_code?: number; parameters?: { retry_after?: number } }; message?: string };
      ...
      failed++;
      if ((i + 1) % 100 !== 0) {
        // Only log individual failures outside of progress lines
      }
    }
  ...
  console.log(`  Failed (deleted/error): ${failed}`);
